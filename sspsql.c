...
#define QUERY(x,y) printf("[sql]:%s\n",y);if (mysql_query((x), (y))) printf("Error %u: %s\n", mysql_errno(x), mysql_error(x))
#define HEADER "id BIGINT NOT NULL AUTO_INCREMENT, kind CHAR(64) NOT NULL, spell CHAR(128), type CHAR(64) NOT NULL, hash BIGINT, primary key (id)" 
...
void 
sql_create_tbl(MYSQL *conn, char *tblname){ 
    char *query = malloc(256 * sizeof(char));
    if (conn == NULL){printf(" Connection Error\n");}
    sprintf(query, "drop table IF EXISTS %s", tblname);
    QUERY(conn, query);
    sprintf(query, "create table %s (%s)", tblname, HEADER);
    QUERY(conn, query);
    free(query);
}
